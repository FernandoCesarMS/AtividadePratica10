...
int main(){
    ...
    cout << "insira quantas contas bancarias deseja criar : " ;
    cin >> numContas;
    ...
    for(int i = 0; i < numContas; i++) //lendo as contas, senhas e saldo
    {
        ...
        if(verifica == 1) //para corrente
        {
            ...
            cin >> auxsenha;
            ContaCorrente auxCorrente(auxsenha);
        ...
        else if(verifica == 2) //pra poupanca
        {
            ...
            cin >> auxsenha;
            ContaPoupanca auxPoupanca(auxsenha);
    ...
    for(int i = 0; i < numContas; i+2)
    {
        if(cp[i] == 1)
        {
            cout << "quantidade de transacoes feitas na conta "<< contas[i]->getNumero() <<" : " << contas[i]->getTransacoes() << endl;
        }
    }
    for(int i = 0; i < numContas; i+2)
    {
